...
class ModelAdapter:
    ...
    def _call_local(self, prompt, system):
        ...
        r = requests.post(
            ...
            json={"model": self.model or os.environ.get("BC_MODEL") or "nvidia/Llama-3.1-Nemotron-Nano-4B-v1.1", "messages": msgs,
                  "max_tokens": 400},
            timeout=30,
        )
        r.raise_for_status()
...
def run(recall_number, db=None, adapter=None, force_alert=False):
    ...
    db = get_db() if db is None else db
    adapter = adapter if adapter is not None else ModelAdapter(backend=os.environ.get("BC_BACKEND","openrouter"), base_url=os.environ.get("NEMOCLAW_URL"), model=os.environ.get("BC_MODEL"))
